...
if(!$sca) {
	...
	$row = sql_fetch(" select wr_id from {$write_table} where wr_is_comment = 0 and ca_name = '{$sca}' order by wr_id desc limit 1 ");
	if (!$row['wr_id']) {
		//goto_url(short_url_clean(G5_HTTP_BBS_URL.'/write.php?bo_table='.$bo_table));
	} else {
		goto_url(short_url_clean(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$row['wr_id']));
...
<div id="board_contents">
	...
	<!-- 게시판 목록 시작 { -->
    <div class="sabasam_board_wrap counseling_txt01">
	<!-- 게시판 카테고리 시작 { -->
	<?php if ($is_category) { ?>
	...
	<!-- } 게시판 카테고리 끝 -->
        ...
        <table class="sabasam_board">
            ...
            ?>
                ...
                ?>
                    ...
                    <td class="sabasam_board_contents">
                        <?php
                        if (0 && $is_category && $list[$i]['ca_name']) {
                        ?>
                    ...
                    <td class="sabasam_board_date"><?php echo str_replace('-', '.', $list[$i]['datetime']) ?></td>
                    <td class="sabasam_board_view"><?php echo $list[$i]['wr_hit'] ?></td>
                </tr>
            <?php } ?>
        ...
        <div class="sabasam_board_footer">
            <div class="sabasam_board_footer_btn">
            <?php if ($is_checkbox) { ?>
            ...
            <!-- 페이지 -->
            <div class="sabasam_board_footer_pagenation">
            <?php echo $write_pages; ?>
            </div>
            <!-- 페이지 -->
            <div class="sabasam_board_footer_write">
